...
export default function AdminDashboard() {
  const { students, sessions, items, stats, loading } = useAdminData()
      ...
      <main className="flex-1 p-6 lg:p-10 max-w-7xl mx-auto w-full">
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Real-time psychometric oversight and system health</p>
          </div>
        ...
        {/* Primary Metrics — now reads from shared context, always current */}
                ...
                (['High', 'Average', 'Low'] as const).map(lvl => {
                  ...
                  return (
                    <div key={lvl} className="space-y-2">
                      ...
                      <p className="text-[10px] font-mono text-slate-400 uppercase">{pct}% of total</p>
                    </div>
                  )
                })
              ) : (
...
function AbilityBadge({ level }: { level: string }) {
  ...
  return (
    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${styles[level] ?? 'bg-slate-50 text-slate-600'}`}>
      {level?.toUpperCase()}
    </span>
  )
}